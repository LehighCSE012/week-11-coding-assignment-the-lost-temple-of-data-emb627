...
def load_artifact_data(excel_filepath):
    ...
    return df
    #Reads the excel file using pandas and returns the resulting DataFrame
...
def load_location_notes(tsv_filepath):
    ...
    return df
    #Reads the tsv file that is tab-separated using pandas and returns the resulting DataFrame
...
def extract_journal_dates(journal_text):
    ...
    """
    dates_list = re.findall(r"\d{2}/\d{2}/\d{4}", journal_text)
    return dates_list
...
def extract_secret_codes(journal_text):
    ...
    """
    secret_codes_list = re.findall(r"AZMAR-\d{3}", journal_text)
    return secret_codes_list
    #Finds all codes that match the specified secret pattern in the journal_text file
    #Returns the list of these secret code strings
